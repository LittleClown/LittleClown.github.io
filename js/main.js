...
//by SparklingWind
  ...
  for(var i=0,len=hl.length;i<len;i++) {
    var $aim = hl.eq(i);
    var lines = hl.eq(i).find(".gutter .line:last").html();
    $aim.before($("<button class='copy-btn' />").append($("<img src='/img/clippy.svg' class='copy-ico'>")));
    $aim.before("<div class='code-toggle'></div>");
    $aim.after("<div class='code-toggle'></div>");

    var $hiding_msg = "+&nbsp;展开代码&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;" + lines + "  行";
    var $showing_msg = "-&nbsp;收起代码";
    var $aim_prev = $aim.prev();

    $aim.next().html($showing_msg).hide();
    $aim_prev.click(function(e){
      var t=$(e.target).next();
      if(t[0].style.display=="none"){
        $(e.target).html($showing_msg);
        t.slideDown("slow");
        if(lines>=20) t.next().show();
      }
      else{
        $(e.target).html($hiding_msg);
        t.next().hide();
        t.slideUp("slow");
      }
    });
    $aim.next().click(function(e){
      var t=$(e.target).prev();
      var lines=parseInt(t.find(".gutter .line:last").html());
      $(e.target).hide();
      t.prev().html($hiding_msg);
      t.slideUp("slow");
    });
    $aim.wrap("<div></div>");

    if( lines > 5 ) {
      $aim_prev.html($hiding_msg);
      $aim.parent().hide();
    } else $aim_prev.html($showing_msg);
  }
